...
const SpecialiteList = () => {
  ...
  const loadSpecialite=async()=>{
    const result=await axios.get("/api/specialites/all");
  ...
  const handleEdit = (id) => {
    ...
    if (newName) {
      axios.put(`/api/specialites/${id}`, {nom:newName }).then(() => {
          ...
          {specialites.map((specialite) => (
            <tr key={specialite.id}>
              ...
              <td>
                                ...
                                {'   '}
                                <button className="btn btn-primary" onClick={() => handleEdit(specialite.id)}>
                                    Edit
                                </button>
                            </td>
            </tr>
